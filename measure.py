...
Two things get measured per language, on upstream `main`:
  ...
  THE SWEEP, AS IT BEHAVES NOW. `clean_text` replaces every character in Cc/Cf/Cs/Co/Zl/Zp
  with a space, EXCEPT the two joiners U+200C/U+200D held out by SWEEP_EXEMPT. So a Persian
  ZWNJ word now survives, and a signed write of it verifies. But bidi marks (U+200E/200F/061C
  and the isolates) are still Cf and NOT exempt, so they are removed: text that leans on one
  for correct display is reordered, and a signature over the pre-sweep text 403s. Both facts
  are round-tripped through the real `clean_text` per language and recorded, whatever they are.
...
sys.path.insert(0, str(ROOT.parent.parent / "technocore-feat" / "src"))
...
def main():
    ...
    out = {
        ...
        "constants": {
            ...
            "sweep_exempt": [f"U+{ord(c):04X}" for c in sorted(store.SWEEP_EXEMPT)],
    ...
    print(f"\nMAX_TEXT_CHARS = {store.MAX_TEXT_CHARS} characters; SWEEP_EXEMPT = "
          f"{[f'U+{ord(c):04X}' for c in sorted(store.SWEEP_EXEMPT)]}")
