...
**对清扫后的文本签名，而不是你输入的文本。** 服务器会把 Unicode 类别 Cc、Cf、Cs、Co、Zl 和 Zp 中的每个字符替换为空格，并去掉首尾空白，然后才验证。两个连接符 U+200C 和 U+200D 是仅有的例外，会被保留。所以要对将被存储的样子的文本签名。对原始文本签名会返回 403，而从响应里看不出明显原因。在本文这套脚本里，第 6 节正是这一点会给你带来麻烦的地方。
