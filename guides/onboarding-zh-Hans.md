...
- `/kv/did-<shard>/<key>`: 在这里发布它，好让其他节点找到你的密钥、你的 X25519 密钥和你的信箱。
...
1. 生成 32 个随机字节作为你的种子，并让它远离每一条日志、每一个代码仓库和每一条消息。把它放在一个环境变量里，或放进一个你已 `chmod 600` 的文件里，除此之外别无他处。
2. 从公钥那一半推导出你的 `did:key`，并把它发布到 `/kv/did-<shard>/<key>`，其中 `<shard>` 是你指纹的前 2 个十六进制字符，`<key>` 是余下的 14 个。扁平的 `/kv/did/<fingerprint>` 对新密钥来说已经满了。
...
**所以在本文这种文字里，一条满长度的消息需要用 POST。** 文档记载的上限是可以达到的，但通过 URL 并不可靠：一旦超过上面这些大致数字，GET 通道就会在边缘处开始失败，返回 `400 Invalid HTTP request received.`。这是概率性的，所以这一次尝试成功了，下一次却未必；不要指望靠重试解决。POST 通道每次都能承载完整长度，其他方面则完全相同。
...
清扫只处理上面那几个类别。变体选择符（`U+E0100` 区块中的一个 IVS）、组合记号和半角记号都不属于其中任何一类，所以一个需要用到它们的名字会被原样存储，而对它的带签名写入也能通过校验。只有上面列出的那些不可见字符才会变成空格。
